    ...
    <div class="middle" @mouseover="over" @mouseout="out">
      <div v-if="msgList.length">
        <div v-for="msg in msgList">
          <div class="msg"
            :style="msg.sendUser === contact.id ? 'flex-direction: row;' : 'flex-direction: row-reverse;'">
            <div class="avatar">
              <img alt="" :src="`${api}/static/img/${msg.from}.jpg`" />
            </div>
            <div v-if="msg.sendUser === contact.id" style="flex: 13;">
    ...
    mounted () {
      ...
      this.interval = setInterval(() => {
        if (this.contact && this.contact.id) {
          ...
          //   url: '/pullMsg',
          //   params: {
          //     from: JSON.parse(localStorage.getItem('currentUser')).id,
          //     to: this.contact.id
          //   }
          // }).then(res => {
